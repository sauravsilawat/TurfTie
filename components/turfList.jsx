...
export default function TurfList() {
    ...
    return (
        <View className='flex-row flex-wrap justify-between gap-y-10 gap-x-2'>
            {filteredData && filteredData.map((item) => (
                <Link href={`/listing/${item.key}`} key={item.key} asChild>
                    <TouchableOpacity className="relative w-[160px]">
                        <Image source={{ uri: item.img }} className="h-36 rounded-t-xl"></Image>
                        ...
                        <View className="p-2 bg-[#00B09D] rounded-b-xl">
                            <Text className="text-white font-extrabold text-sm">{item.title}</Text>
                            <Text className="text-white text-xs">{item.loc}</Text>
                        </View>
                    </TouchableOpacity>
                </Link>
            ))
